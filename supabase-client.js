...
window.MeetlyData = (() => {
  ...
  const adapter = {
    ...
    signInWithGoogle: () => {
      // Keep the invite deep-link across the OAuth round-trip, which drops the query string.
      // Clear on a plain sign-in too, so an abandoned invite login cannot leak into it.
      if (location.search) sessionStorage.setItem(pendingInviteKey, location.search);
    ...
    finishGoogleLogin: async () => {
      ...
      history.replaceState(null, '', pendingInvite
        ? `${location.pathname}${pendingInvite}#respond`
